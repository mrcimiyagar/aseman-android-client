...
class SlidingPanel(context: Context, attrs: AttributeSet? = null) : FrameLayout(context, attrs) {
    ...
    override fun drawChild(canvas: Canvas, child: View, drawingTime: Long): Boolean {
        ...
        when (child) {
            nonSlidingView -> {
                //maxSlide =
                //    if (isOrientationVertical()) nonSlidingView.height.toFloat() - resources.displayMetrics.density.toInt() * 112
                //    else nonSlidingView.width.toFloat()
