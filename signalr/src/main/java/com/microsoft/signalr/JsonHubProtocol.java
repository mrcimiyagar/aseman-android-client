...
class JsonHubProtocol implements HubProtocol {
    ...
    private ConfigFetchigController configPresenter;

    public JsonHubProtocol(ConfigFetchigController configPresenter) {
    ...
    public HubMessage[] parseMessages(String payload, InvocationBinder binder) {
        ...
        if (this.configPresenter.isLoggerEnabled())
            LogHelper.log("SignalRMessage", "Received data : " + payload);
        ...
        for (String str : messages) {
            ...
            if (configPresenter.jsonConverterType() == JsonConverterType.GSON) {
                ...
                try {
                    ...
                    factory.setCodec(JsonHelper.getMapper());
                    ...
                    do {
                        ...
                        switch (name) {
                            ...
                            case "item":
                                ...
                                break;
                            case "arguments":
                                LogHelper.log("SignalRTemp", "arguments detected ****************************************");
                                if (target != null) {
                                    try {
                                        parser.nextToken();
                                        List<Class<?>> types = binder.getParameterTypes(target);
                                        ...
                                        for (int counter = 0; counter < types.size(); counter++)
                                            arguments.add(JsonHelper.getMapper().treeToValue(tn.get(counter), types.get(counter)));
                                    } catch (Exception ex) {
                                        argumentBindingException = ex;
                                        ex.printStackTrace();
                                    }
